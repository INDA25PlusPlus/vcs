...
fn format_status(status: &RepoStatus<Digest>) -> String {
    ...
    append_section(&mut output, "Staged changes", status.staged.changes());
    append_section(&mut output, "Pending changes", status.pending.changes());
    output
}
...
fn append_section(
    output: &mut String,
    title: &str,
    changes: BTreeMap<RepoPath, FileChange<Digest>>,
) {
    if changes.is_empty() {
        return;
    }
